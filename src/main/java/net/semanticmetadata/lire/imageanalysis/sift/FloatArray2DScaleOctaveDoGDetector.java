...
public class FloatArray2DScaleOctaveDoGDetector {
	...
	private static final float MAX_CURVATURE_RATIO = (MAX_CURVATURE + 1) * (MAX_CURVATURE + 1)
			/ MAX_CURVATURE;
	...
	public void run(final FloatArray2DScaleOctave o) {
		this.octave = o;
		this.candidates = new Vector<float[]>();
		detectCandidates();
	}
				...
				X: for (int x = d[i].width - 2; x >= 1; --x) {
					...
					do {
						...
						Array2DRowRealMatrix H = new Array2DRowRealMatrix(new double[][] {
								{ dxx, dxy, dxi }, { dxy, dyy, dyi }, { dxi, dyi, dii } });
						RealMatrix H_inv;
						try {
						...
						if (odc < 2.0f) {
							if ((Math.abs(ox) > 0.5 || Math.abs(oy) > 0.5 || Math.abs(oi) > 0.5)
								...
								if (xc < 1 || yc < 1 || ic < 1 || xc > d[0].width - 2
										|| yc > d[0].height - 2 || ic > d.length - 2) {
									isLocalizable = false;
								} else {
									xa = xc - 1;
									xb = xc + 1;
								...
								if (fx < 0 || fy < 0 || fi < 0 || fx > d[0].width - 1
										|| fy > d[0].height - 1 || fi > d.length - 1) {
									isLocalizable = false;
								} else {
									isLocalized = true;
								}
